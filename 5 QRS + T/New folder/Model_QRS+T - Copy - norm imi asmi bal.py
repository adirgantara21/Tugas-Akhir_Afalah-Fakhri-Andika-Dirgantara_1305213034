...
# Define class names for later metrics and confusion matrix
class_names = ['NORM'] + mi_subclasses

# Select feature columns (exclude IDs, labels, and ST features)
feature_cols = [c for c in df.columns if c not in ["beat_id","patient_id","record_id","label","is_mi"] + mi_subclasses
                and "st_elevation" not in c.lower()
                and "st_depression" not in c.lower()]

# ========= Undersampling =========
# Batasi kelas NORM, IMI, dan AMI menjadi 4000 sampel masing-masing
undersample_classes = ['NORM', 'IMI', 'ASMI']
# Gabungkan fitur, label, dan class identifier
...
for cls in undersample_classes:
    ...
    if len(cls_df) >= 4000:
        dfs.append(cls_df.sample(n=4000, random_state=42))
    else:
        dfs.append(cls_df)
# Sisanya (kelas lain) tetap utuh
df_remaining = df_all[~df_all['label'].isin(undersample_classes)]

# Gabung dan acak dataset baru
df_bal = pd.concat(dfs + [df_remaining]).sample(frac=1, random_state=42)
...
# Struktur penyimpanan metrik dan CM
...
# Untuk menyimpan AUC dan TPR ROC
train_auc = {cls: [] for cls in class_names}
train_interp_tprs = {cls: [] for cls in class_names}
test_auc = {cls: [] for cls in class_names}
test_interp_tprs = {cls: [] for cls in class_names}
...
for fold, (tr, te) in enumerate(kf.split(X, y[:,0]), start=1):
    ...
    model = ClassifierChain(
        base_estimator=XGBClassifier(n_estimators=250, max_depth=7, learning_rate=0.1,
                                    subsample=0.8, colsample_bytree=0.8, eval_metric='logloss'),
        order='random', random_state=42
    )
    model.fit(X_tr, y_tr)
# ==== TRAIN ROC AUC ====
y_train_score = model.predict_proba(X_tr)
if isinstance(y_train_score, list):
    y_train_score = np.vstack([p[:,1] for p in y_train_score]).T

mean_fpr = np.linspace(0,1,100)
for idx, cls in enumerate(class_names):
    fpr_tr, tpr_tr, _ = roc_curve(y_tr[:,idx], y_train_score[:,idx])
    train_auc[cls].append(roc_auc_score(y_tr[:,idx], y_train_score[:,idx]))
    train_interp_tprs[cls].append(np.interp(mean_fpr, fpr_tr, tpr_tr))
...
for idx, cls in enumerate(class_names):
    ...
    test_auc[cls].append(roc_auc_score(y_te[:,idx], y_score[:,idx]))
    test_interp_tprs[cls].append(np.interp(mean_fpr, fpr, tpr))
    ...
    if mean_fold_auc > best_auc:
        ...
        joblib.dump(model, 'models/best_model_QRS+T.pkl')
...
# ==== PLOT ROC: TRAINING vs TESTING ====
plt.figure(figsize=(10, 8))
mean_fpr = np.linspace(0,1,100)

for cls in class_names:
    # Test ROC
    mean_test_tpr = np.mean(test_interp_tprs[cls], axis=0)
    mean_test_auc = np.mean(test_auc[cls])
    plt.plot(mean_fpr, mean_test_tpr, label=f'{cls} Test AUC={mean_test_auc:.2f}', linestyle='-')

    # Train ROC
    mean_train_tpr = np.mean(train_interp_tprs[cls], axis=0)
    mean_train_auc = np.mean(train_auc[cls])
    plt.plot(mean_fpr, mean_train_tpr, label=f'{cls} Train AUC={mean_train_auc:.2f}', linestyle='--')

plt.plot([0, 1], [0, 1], 'k--')
plt.title('ROC Curves: Training vs Testing per Class')
